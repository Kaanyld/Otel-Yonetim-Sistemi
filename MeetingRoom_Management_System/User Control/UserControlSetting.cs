...
namespace MeetingRoom_Management_System.User_Control
{
    public partial class UserControlSetting : UserControl
    {
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                check = db.AddUser(textBoxUser.Text.Trim(), textBoxPassword.Text.Trim());
                if(check) 
                    Clear();
            }
        }
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            ...
            if ( ID !="")
            {
                if (textBoxUser1.Text.Trim() == string.Empty || textBoxPassword1.Text.Trim() == string.Empty)
                    MessageBox.Show("Lütfen boş alanları doldurunuz.", "Tüm alanlar doldurulmalı", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else
                {
                    check = db.UpdateUser(ID,textBoxUser1.Text.Trim(), textBoxPassword1.Text.Trim());
                    if (check)
                        Clear1();
                }
            }
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            if (ID != "")
            {
                ...
                {
                    DialogResult result = MessageBox.Show("Bu kullanıcıyı silmek istediğine emin misin?", "Kullanıcıyı Sil", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void dataGridViewUser_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex!=-1)
            {
                ...
                textBoxUser1.Text = row.Cells[1].Value.ToString();
        ...
        {
            db.DisplayAndSearch("SELECT * FROM User_Table WHERE User_Name LIKE '%" + textBoxSearchUser.Text + "%'", dataGridViewUser);
        }
    }
}
