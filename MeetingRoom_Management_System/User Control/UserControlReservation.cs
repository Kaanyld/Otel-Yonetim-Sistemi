...
namespace MeetingRoom_Management_System.User_Control
{
    public partial class UserControlReservation : UserControl
    {
        ...
        {
            db.DisplayAndSearch("SELECT * FROM Reservation_Table WHERE Client_Phone LIKE '%" + textBoxSearchClientID.Text + "%'", dataGridViewReservation);
        ...
        private void buttonCancel_Click(object sender, EventArgs e)
        {
            ...
            if (RID != "")
            {
                if (comboBoxType1.Text.Trim() == string.Empty || textBoxClientID1.Text.Trim() == string.Empty)
                    MessageBox.Show("Lütfen boş alanları doldurunuz.", "Tüm alanlar doldurulmalı", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else
                {
                    DialogResult result = MessageBox.Show("Bu müşteriyi silmek istediğine emin misin?", "Rezervasyon Sil", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
